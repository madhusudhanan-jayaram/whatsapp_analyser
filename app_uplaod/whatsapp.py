...
class WhatsappFilrt:
    ...
    def PreProcessing(self,filename):
        df = pd.read_csv(filename,header=None,encoding='utf8',on_bad_lines='skip')
        if(len(df.columns)==2):
            df=df.drop(0)
            df=df.dropna(axis=0)
    ...
    def give_emoji_free_text(self,text):
        allchars = [c for c in text.decode('utf-8')]
        emoji_list = [c for c in allchars if c in emoji.EMOJI_DATA]
        clean_text = ' '.join([s for s in text.decode('utf-8').split() if not any(i in s for i in emoji_list)])
        return clean_text
