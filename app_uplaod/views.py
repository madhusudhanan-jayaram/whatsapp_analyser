...
class FileUploadView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if txtfile.name.endswith('.txt'):
            ...
            #print('inside save')
            try:
                df = pd.read_csv(file_path,header=None,encoding='utf8',on_bad_lines='skip')
                #return df
            except FileNotFoundError:
                return HttpResponse('File not found. Please check your file name.')
            except pd.errors.ParserError as e:
                return HttpResponse(f'Error parsing file: {str(e)}. Please check your file format or try cleaning the file.')
            obj=WhatsappFilrt()
